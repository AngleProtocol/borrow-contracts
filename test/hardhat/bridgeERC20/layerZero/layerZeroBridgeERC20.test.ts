...
contract('LayerZeroBridgeERC20', () => {
  ...
  let lzBridge: LayerZeroBridgeERC20;
  ...
  beforeEach(async () => {
    ...
    lzBridge = (await deployUpgradeable(new LayerZeroBridgeERC20__factory(deployer))) as LayerZeroBridgeERC20;
    ...
    await lzBridge.initialize('LayerZero Bridge Token', lzEndpoint.address, coreBorrow.address, agToken.address);
  });
  ...
  describe('initializer', () => {
    ...
    it('reverts - already initialized', async () => {
      await expect(
        lzBridge.initialize('LayerZero Bridge Token', lzEndpoint.address, coreBorrow.address, agToken.address),
      ).to.be.revertedWith('Initializable: contract is already initialized');
    });
    it('reverts - zero address', async () => {
      const lzBridgeRevert = (await deployUpgradeable(
        new LayerZeroBridgeERC20__factory(deployer),
      )) as LayerZeroBridgeERC20;
      await expect(
        lzBridgeRevert.initialize('LayerZero Bridge Token', ZERO_ADDRESS, coreBorrow.address, agToken.address),
      ).to.be.revertedWith('ZeroAddress');
      await expect(
        lzBridgeRevert.initialize('LayerZero Bridge Token', lzEndpoint.address, ZERO_ADDRESS, agToken.address),
      ).to.be.revertedWith('ZeroAddress');
      await expect(
        lzBridgeRevert.initialize('LayerZero Bridge Token', lzEndpoint.address, coreBorrow.address, ZERO_ADDRESS),
      ).to.be.revertedWith('ZeroAddress');
      await expect(
        lzBridgeRevert.initialize('LayerZero Bridge Token', ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS),
      ).to.be.revertedWith('ZeroAddress');
    });
  });
  describe('name', () => {
