...
contract('Swapper', () => {
  ...
  beforeEach(async () => {
    ...
    swapper = (await new SwapperSidechain__factory(deployer).deploy(
      core.address,
      router.address,
      router.address,
      router.address,
  ...
  describe('constructor', () => {
    ...
    it('reverts - zero address', async () => {
      await expect(
        new SwapperSidechain__factory(deployer).deploy(ZERO_ADDRESS, router.address, router.address, router.address),
      ).to.be.revertedWith('ZeroAddress');
      await expect(
        new SwapperSidechain__factory(deployer).deploy(core.address, ZERO_ADDRESS, router.address, router.address),
      ).to.be.revertedWith('ZeroAddress');
      await expect(
        new SwapperSidechain__factory(deployer).deploy(core.address, router.address, ZERO_ADDRESS, router.address),
      ).to.be.revertedWith('ZeroAddress');
      await expect(
        new SwapperSidechain__factory(deployer).deploy(core.address, router.address, router.address, ZERO_ADDRESS),
      ).to.be.revertedWith('ZeroAddress');
    });
  });
  ...
  describe('swap - 1Inch', () => {
    ...
    it('success - correct amount out twice 1/3', async () => {
      ...
      const data = ethers.utils.defaultAbiCoder.encode(
        ['address', 'address', 'uint256', 'uint128', 'uint128', 'bytes'],
        [ZERO_ADDRESS, ZERO_ADDRESS, 0, 1, 0, payload1inch],
      );
      await swapper.swap(collateral.address, stablecoin.address, alice.address, parseEther('1'), parseEther('1'), data);
      await collateral.mint(swapper.address, parseEther('1'));
      await swapper.swap(collateral.address, stablecoin.address, alice.address, parseEther('1'), parseEther('1'), data);
