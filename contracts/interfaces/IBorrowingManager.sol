...
/// @notice Parameters associated to a given `BorrowingManager` contract: these all correspond
/// to parameters which signification is detailed in the `BorrowingManagerStorage` file
...
struct Vault {
    ...
    // Normalized value of the debt (that is to say of the assets borrowed). It is expressed
    // in the base of the asset borrowed
...
struct LiquidationOpportunity {
    // Maximum asset amount that can be repaid upon liquidating the vault
    uint256 maxAssetAmountToRepay;
    ...
    // Threshold value of asset amount to repay: it is ok for a liquidator to repay below threshold,
    // but if this threshold is non null and the liquidator wants to repay more than threshold, it should repay
    // the max asset amount given in this vault
...
struct LiquidatorData {
    // Current amount of assets the liquidator should give to the contract
    uint256 assetAmountToReceive;
    ...
    // Oracle value (in asset base) at the time of the liquidation
    uint256 oracleValue;
    // Value of the `interestAccumulator` at the time of the call
    uint256 newInterestAccumulator;
}

/// @notice Data to track during a series of action the amount to give or receive in assets and collateral
/// to the caller or associated addresses
struct PaymentData {
    // Stablecoin amount the contract should give
    uint256 assetAmountToGive;
...
/// @title IBorrowingManagerFunctions
/// @author Angle Labs, Inc.
/// @notice Interface for the `BorrowingManager` contract
/// @dev This interface only contains functions of the contract which are called by other contracts
/// of this module (without getters)
interface IBorrowingManagerFunctions {
    /// @notice Gets the current debt of a vault
    /// @param vaultID ID of the vault to check
    /// @return Debt of the vault
    function getBorrowDebt(uint256 vaultID) external view returns (uint256);
    ...
    /// @param datas Data to be decoded for each action: it can include like the `vaultID` or the `assetAmount` to borrow
    /// @param from Address from which assets will be taken if one action includes burning assets. This address
    /// should either be the `msg.sender` or be approved by the latter
    /// @param to Address to which assets and/or collateral will be sent in case of
    /// @param who Address of the contract to handle in case of repayment of assets from received collateral
    ...
    /// a positive amount of assets to receive as well as a positive amount of assets to give
    ...
    /// it is encouraged to use `vaultID = 0` only when the first action of the batch is `createBorrow`
    function angle(
...
/// @title IBorrowingManagerStorage
/// @author Angle Labs, Inc.
/// @notice Interface for the `BorrowingManager` contract
/// @dev This interface contains getters of the contract's public variables used by other contracts
/// of this module
interface IBorrowingManagerStorage {
    /// @notice Encodes the maximum ratio asset/collateral a vault can have before being liquidated. It's what
    /// determines the minimum collateral ratio of a position
    function collateralFactor() external view returns (uint64);

    /// @notice Stablecoin handled by this contract. Another `BorrowingManager` contract could have
    /// the same rights as this `BorrowingManager` on the asset contract
    function asset() external view returns (address);

    /// @notice Oracle contract to get access to the price of the collateral with respect to the asset
    ...
    /// @notice Reference to the collateral handled by this `BorrowingManager`
    function collateral() external view returns (IERC20);

    /// @notice Total normalized amount of assets borrowed, not taking into account the potential bad debt accumulated
    /// This value is expressed in the base of Angle assets (`BASE_TOKENS = 10**18`)
    function totalNormalizedDebt() external view returns (uint256);

    /// @notice Maximum amount of assets that can be issued with this contract. It is expressed in `BASE_TOKENS`
...
/// @author Angle Labs, Inc.
/// @notice Interface for the `BorrowingManager` contract
interface IBorrowingManager is IBorrowingManagerFunctions, IBorrowingManagerStorage, IERC721Metadata {
    function isApprovedOrOwner(address spender, uint256 vaultID) external view returns (bool);
}

/// @title IBorrowingManagerListing
/// @author Angle Labs, Inc.
/// @notice Interface for the `BorrowingManagerListing` contract
interface IBorrowingManagerListing is IBorrowingManager {
