...
contract VaultManagerListing is VaultManager {
    ...
    /// @dev Checkpoints the staker associated to the `collateral` of the contract before an update of the
    /// `collateralAmount` of vaultID
    /// @dev It's important that this function is called right before an update of the `collateralAmount` of the
    /// vault otherwise the staker contract may send too many rewards to an address and not enough to another one
